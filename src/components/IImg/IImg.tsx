...
const IImg = defineComponent({
  ...
  setup(props){
    ...
    if(lazy.value){
      const {cleanup} = useIntersectionObserver(root,([{isIntersecting}]) => {
    ...
    const imgStyle = computed(() => {
      const __width = isType(width.value,'Number') ? `${width.value}px` : width.value
      const __height = isType(height.value,'Number') ? `${height.value}px` : height.value
      if(__width || __height) {
        return {
          width:__width,
          height: __height,
